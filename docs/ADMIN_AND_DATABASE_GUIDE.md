# Управление базой данных, администраторами и безопасностью (v0.8.7)
...
Панель открывается по защищённому протоколу HTTPS через шлюз Caddy (с защитой WAF и двухфакторной аутентификацией 2FA):
```
https://yenotick.duckdns.org/
```
...
3. **Единый Error ID**:
   ...
   - Администратор может мгновенно локализовать проблему по коду:
     ...
     ```
4. **Telegram-бот мониторинга (`tg-monitor`)**:
   - Специализированный фоновый сервис для главного администратора (`TELEGRAM_ADMIN_ID`).
   - Собирает нагрузку хоста (CPU, RAM, диск, аптайм) и здоровье всех контейнеров Docker.
   - Автоматически отправляет алерт при сбое контейнера или переходе в `unhealthy` с кнопкой «🔄 Перезапустить».
   - Команды бота:
     - `/status` — ресурсы хоста и статус контейнеров.
     - `/restart` — меню выбора контейнера или перезапуска всех сервисов.
     - `/logs` — просмотр последних логов контейнера.
     - `/reboot` — перезагрузка физического сервера с подтверждением.
