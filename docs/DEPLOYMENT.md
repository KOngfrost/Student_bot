...
Веб-панель делит сетевой стек с Tailscale-контейнером и НЕ публикуется на хост.
...
|---|---|---|
| `WEB_WORKERS` | 4 | Равен числу vCPU; больше не даёт выигрыша |
...
- **PostgreSQL** - индексы по статусам и датам заявок уже созданы миграциями.
  ...
  тысяч пользователей.
- **Веб-панель** - несколько uvicorn-воркеров (`WEB_WORKERS`). Статика
