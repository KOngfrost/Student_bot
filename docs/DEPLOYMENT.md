...
Архитектура v0.7.9.9 полностью готова к горизонтальному масштабированию благодаря распределённому хранилищу Redis:
...
- **Advisory Locks для Outbox**: доставка сообщений из очереди координируется через `pg_try_advisory_lock` в PostgreSQL (`core/task_dispatcher.py`). Даже при десятках запущенных инстансов веб-панели и бота одно и то же сообщение никогда не будет отправлено дважды.
