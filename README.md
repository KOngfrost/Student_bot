...
## Ключевые возможности v0.8.2

- **Премиальная дизайн-система Glassmorphism и Rose Peony палитра**:
  - Настоящий эффект матового стекла (`backdrop-filter: blur(14px)`), полупрозрачные карточки (`rgba(255, 255, 255, 0.78)` в светлой теме) с мягким белым кантом (`--border-glass`).
  - Благородная гармония светлой темы: мягкий шелковистый розовый градиент пиона (`#e85584` → `#cc3363`) с деликатным внутренним бликом и рассеянной тенью вместо резкого неона `#ff2d87`, чистый фарфорово-белый фон (`#fcf9fa`) без грязной розовой дымки.
- **Строгий деловой стиль без лишних эмодзи**:
  - Полное удаление эмодзи из веб-панели управления, шаблонов, заголовков и служебных скриптов.
  - Сохранение эмодзи исключительно в ответах VK-бота для теплоты и дружелюбности общения со студентами.
- **Редизайн сайдбара и профиля оператора**:
  - Новый сквиркл-аватар с первой буквой имени администратора, плашка роли (`Суперадмин`), аккуратные SVG-кнопки перехода в настройки и выхода.
  - Замена всех текстовых псевдосимволов навигации на чёткие векторные SVG-иконки.
  - Удалена нестилизованная кнопка переключения темы и скрыт мобильный крестик `✕` на мониторах ПК.
- **Исправление 2FA и фоновой синхронизации**:
  - Устранён дедлок автоотправки формы в `2fa.html`, приводивший к бесконечному статусу «Проверка...» при вводе 6 цифр.
  - Корректная обработка `401 Unauthorized` в `store.js` для предотвращения циклических фоновых запросов.
- **Модуль мероприятий (Events) и регистраций**:
  - Устранена ошибка `AttributeError: type object 'Event' has no attribute 'is_active'` в VK-хендлере.
  - Корректная выборка актуальных событий по дате проведения (`event_date >= now - timedelta(hours=3)`).
  - Поддержка вывода описания мероприятия в сообщении бота и расширение списка синонимов («Мероприятия», «События», «Афиша», «/events»).
  - Добавлен автоматический пинг heartbeat при вызове событийных команд и полное покрытие юнит-тестами.

## Ключевые возможности v0.8.1

- **Startup Guard и Bootstrap Guard**: fail-closed валидация секретов при старте в `production` (`core/startup_guard.py`, `core/bootstrap_guard.py`), блокировка запуска при наличии словарных/placeholder паролей (`admin123`, `changeme` и т.д.).
- **Харденинг 2FA и защита от подбора OTP**:
  - Строгий лимит неверных попыток ввода одноразового кода (`WEB_ADMIN_2FA_MAX_ATTEMPTS = 5`) и лимит повторной отправки OTP (`WEB_ADMIN_2FA_RESEND_LIMIT = 3`).
  - Криптографическое хранение OTP с хешированием Argon2id в распределённом Redis (`web/security/otp_store.py`).
  - Честные HTTP-статусы `401 Unauthorized` и `429 Too Many Requests` с заголовком `Retry-After`.
- **Интернационализация (i18n)**: архитектура мультиязычности (`core/i18n.py`, `babel.cfg`, `web/templating.py` через `install_gettext_callables`), поддержка переключения языков (RU/EN) и безопасный fallback.
- **Ускорение поиска (PostgreSQL pg_trgm GIN)**: миграция `c4d5e6f7a8b9_add_pg_trgm_gin_indexes.py` с триграммными GIN-индексами для быстрого нечёткого поиска по обращениям и базе знаний.
- **Целостность данных (ON DELETE CASCADE/SET NULL)**: миграция `b5a7c9d1e2f3_enforce_fk_ondelete.py` для исключения рассинхронизации и "висячих" записей в БД.
- **Disaster Recovery под Windows и Linux**: полнофункциональные PowerShell-скрипты резервного копирования и восстановления (`scripts/backup.ps1`, `scripts/restore.ps1`) с проверкой контрольных сумм SHA-256.
...
- **Защита системного журнала**: раздел «Журнал» и экспорт логов аудита в CSV доступны исключительно суперадминистраторам (`SUPERADMIN`).
- **Синхронизация времени**: автоматический контроль дрейфа часов между приложением и СУБД PostgreSQL в `/health` (порог 5 сек).
...
- **Политика обработки персональных данных**: полное соответствие законодательству РФ (`/legal/privacy`, `/legal/consent`, `/legal/terms`).
