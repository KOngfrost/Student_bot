...
## 🔑 Ключевые возможности v0.7.9.9

- 🧩 **Модульная архитектура VK-бота**: разделение монолитного bot.py на доменные пакеты `bots/vk/handlers/` на базе `BotLabeler` (студенты, администраторы, события, FAQ, база знаний, отчёты) с O(1) поиском по локальным номерам заявок.
- 🛡️ **Защита от инъекций и пентестинг**: встроенный комплексный набор тестов безопасности против SQLi, XSS, Buffer Overflow (>65k символов), Null-байтов и несанкционированного межведомственного доступа к чужим заявкам (IDOR/BOLA).
- ⚙️ **Строгие настройки Pydantic**: переход на `pydantic_settings.BaseSettings` с валидацией типов, AliasChoices и безопасными кастомными парсерами окружения.
- ⚡ **Оптимизация Outbox и In-Memory кэша**: 2-фазная пакетная доставка сообщений в VK ровно за 2 сессии БД (вместо $1+2N$) и интервальная очистка кэша без $O(N)$ блокировок.
