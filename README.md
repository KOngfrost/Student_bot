...
- **Мониторинг сервера в Telegram:** круглосуточный бот для главного администратора: системные ресурсы (CPU/RAM/диск), статус контейнеров, авто-алерты при сбоях и перезапуск сервисов в один клик.
...
| **VK Bot** | `vkbottle` 4.x (Long Poll / Callback API) | Приём сообщений и взаимодействие со студентами в VK |
| **Telegram Monitor** | `aiogram` 3.x, Docker Socket UDS, `psutil` | Мониторинг хоста, алерты о сбоях и перезапуск контейнеров |
...
student_bot/
├── bots/                  # Сервисы ботов
│   ├── vk/handlers/       # VK-бот (тикеты студентов, меню, FAQ, события)
│   └── telegram/          # Telegram-бот мониторинга хоста и управления контейнерами
...
├── scripts/               # Скрипты управления (создание пользователей, бэкапы, мониторинг)
...
## Telegram-бот мониторинга сервера

В проект встроен сервис круглосуточного мониторинга сервера `tg-monitor` только для главного администратора:

- **Авторизация**: бот отвечает **только** на сообщения пользователя с `TELEGRAM_ADMIN_ID`.
- **Команды**:
  - `📊 /status` — вывод текущей нагрузки хоста (CPU, RAM, диск, аптайм) и здоровье контейнеров (🟢/🔴/🟡).
  - `🔄 /restart` — меню быстрого перезапуска отдельных контейнеров или всего стека в один клик.
  - `📋 /logs` — просмотр свежих логов контейнера.
  - `⚠️ /reboot` — перезагрузка физического сервера с подтверждением.
- **Авто-алерты**: мгновенное уведомление в Telegram при сбое или переходе любого сервиса в статус `unhealthy` с кнопкой быстрого перезапуска.
