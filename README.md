...
Реализованы: базовое меню, регистрация пользователей, единая модель прав доступа
(таблица `admins`), Excel-отчеты в VK и по email, логирование действий,
защита анонимных обращений и timezone-aware даты.
...
- **Прав доступа**: администраторы определяются через таблицу `admins`. Для быстрого
  тестирования можно добавить VK ID в `ADMIN_VK_IDS` в `.env`.
...
- **Анонимность**: в отчете «Детализация» для анонимных заявок ФИО и общежитие
  маскируются.
- **Логирование**: все действия пользователей (нажатия кнопок, генерация отчетов)
  записываются в таблицу `logs`.
- **Часовые пояса**: все даты хранятся в UTC, отчёты формируются с учетом
...
- **Healthcheck**: PostgreSQL проверяет готовность через `pg_isready`.
  Бот запускается только после готовности базы.
