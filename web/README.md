# Модуль веб-панели (v0.8.8.3)

Веб-панель построена на FastAPI, Jinja2 и современной дизайн-системе Glassmorphism (тёмная графитовая тема с неоновыми розовыми акцентами и светлая фарфоровая тема с благородными розовыми тонами). В продакшене запускается в многоворкерном режиме:
```bash
...
- `static/`:
  - `style.css` — тёмная графитовая и светлая фарфорово-розовая темы с глубоким эффектом матового стекла (`backdrop-filter: blur(14px)`), благородными градиентами, адаптивной сеткой и доступными контрастами.
  - `app.js`, `store.js` — клиентский JavaScript: живой поиск без перезагрузки, фильтры-пилюли, модальные окна, мобильное бургер-меню, логика согласия на cookie, обработка 401 unauthenticated.
...
- `templates/` — шаблоны страниц (Jinja2) со строгим отсутствием эмодзи в служебном UI и чистыми векторными SVG-иконками.
